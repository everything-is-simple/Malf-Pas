...
| `H:\Malf-Pas-data` | Malf-Pas 重构后当前系统本地数据库根目录；后续历史大账本与子库落点；当前 live 授权只允许写入 `market_base_day.duckdb / market_base_week.duckdb / market_base_month.duckdb / market_meta.duckdb` |
...
1. 先有权威设计，再有实现。
2. 首张治理路线图已收口；当前 live route 只打开 `Data Foundation only` 窄授权，且仅允许 `market_base_day.duckdb / market_base_week.duckdb / market_base_month.duckdb / market_meta.duckdb`。
...
7. 首张治理 roadmap 收口后，后续必须新开独立 roadmap，不得继续占用已经 `none / terminal` 的路线图。
8. 每一张后续 roadmap 必须对应一个模块数据库；当前模块库未 ready、未通过检查前，不得开启下一张 roadmap。
9. 每张后续 roadmap 的 ready 必须同时满足 `development_usable = true` 和 `daily_usable = true`；只够开发、不够日常使用，不得进入下一张 roadmap。
10. `daily_usable` 至少要有 ledger、daily incremental 或显式等价机制、dirty scope、checkpoint/resume、freshness/audit 闭环。
11. 正式输入真值优先来自 `H:\tdx_offline_Data` 与 `H:\new_tdx64`；`TuShare / baostock / AKShare` 不得成为正式 truth owner。
12. `H:\Asteria-data` 只允许作为 Data Foundation 的只读 bootstrap 参考；后续模块数据库都属于同一个历史大账本的受治理分账本。

## 当前权威资产

当前权威输入仍以只读消费为主；当前正式输出已扩大到 `H:\Malf-Pas-data\raw_market.duckdb`、`market_base_day/week/month.duckdb` 与第 23 卡新增的 `market_meta.duckdb`：
...
H:\Malf-Pas-Validated\MALF_Three_Part_Design_Set_v1_5 (successor authority design set)
H:\Malf-Pas-Validated\PAS__Three_Part_Design_Set_v1_2 (successor authority design set)
H:\Malf-Pas-Validated\MALF_PAS_Scenario_Atlas_v1_0 (frozen companion atlas)
...
```text
MALF v1.4 is the current authority anchor in H:\Malf-Pas-Validated; H:\Asteria-Validated remains predecessor/original reference only.
PAS v1.1 design set is the current predecessor PAS authority in H:\Malf-Pas-Validated.
...
Formal DB mutation is narrowed to `Data Foundation only`; current live write scope is `H:\Malf-Pas-data\market_base_day.duckdb / market_base_week.duckdb / market_base_month.duckdb / market_meta.duckdb` only.
...
Roadmap 2 is now active-after-card-023 on the `data-foundation` live route.
Data Foundation card 21 raw-market-full-build-ledger-card is closed.
Data Foundation card 22 market-base-day-week-month-build-card is closed.
Data Foundation card 23 market-meta-tradability-calendar-card is closed.
`raw_market.duckdb` now contains `source_file / raw_bar / ingest_run / reject_audit / source_manifest / schema_version`.
`market_base_day/week/month.duckdb` now contain `base_bar / latest_pointer / base_run / dirty_scope / source_manifest / schema_version`.
`market_meta.duckdb` now contains `instrument_master / trade_calendar / tradability_fact / industry_block_relation / source_manifest / schema_version`.
...
8. [post-terminal 路线图与模块库纪律](docs/00-governance/05-post-terminal-roadmap-and-module-db-discipline-v1.md)
9. [主线权威图](docs/01-architecture/00-mainline-authoritative-map-v1.md)
10. [MALF v1.4 锚点位置](docs/01-architecture/01-malf-v1-4-anchor-position-v1.md)
11. [旧系统强项地图](docs/01-architecture/02-predecessor-strength-map-v1.md)
12. [系统主线模块所有权](docs/01-architecture/03-system-mainline-module-ownership-v1.md)
13. [存储引擎与便携性裁决](docs/01-architecture/04-storage-engine-and-portability-decision-v1.md)
14. [历史大账本拓扑协议](docs/01-architecture/05-historical-ledger-topology-protocol-v1.md)
15. [每日增量与断点续传协议](docs/01-architecture/06-daily-incremental-and-resume-protocol-v1.md)
16. [回测窗口与留出样本协议](docs/01-architecture/07-backtest-window-and-holdout-protocol-v1.md)
17. [开源 adapter 边界](docs/01-architecture/08-open-source-adapter-boundary-v1.md)
18. [模块设计文档标准](docs/02-modules/00-module-design-document-standard-v1.md)
19. [PAS 公理化状态机](docs/02-modules/01-pas-axiomatic-state-machine-v1.md)
20. [MALF v1.5 wave behavior snapshot](docs/02-modules/02-malf-v1-5-wave-behavior-snapshot-v1.md)
