...
| `H:\Malf-Pas-data` | Malf-Pas 重构后当前系统本地数据库根目录；后续历史大账本与子库落点；当前 live 授权只允许写入 `raw_market.duckdb` |
...
1. 先有权威设计，再有实现。
2. 首张治理路线图已收口；当前 live route 只打开 `Data Foundation only` 窄授权，且仅允许 `raw_market.duckdb`。
...
当前权威输入仍以只读消费为主；第 21 卡新增的正式输出仅限 `H:\Malf-Pas-data\raw_market.duckdb`：
...
Formal DB mutation is narrowed to `Data Foundation only`; current live write scope is `H:\Malf-Pas-data\raw_market.duckdb` only.
...
Roadmap 2 is now active-after-card-021 on the `data-foundation` live route.
