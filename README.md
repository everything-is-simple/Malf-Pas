...
| `H:\Malf-Pas-data` | Malf-Pas 重构后当前系统本地数据库根目录；后续历史大账本与子库落点；当前 live 授权只允许写入 `market_base_day.duckdb / market_base_week.duckdb / market_base_month.duckdb` |
...
1. 先有权威设计，再有实现。
2. 首张治理路线图已收口；当前 live route 只打开 `Data Foundation only` 窄授权，且仅允许 `market_base_day.duckdb / market_base_week.duckdb / market_base_month.duckdb`。
...
当前权威输入仍以只读消费为主；当前正式输出已扩大到 `H:\Malf-Pas-data\raw_market.duckdb` 与第 22 卡新增的 `market_base_day/week/month`：
...
Formal DB mutation is narrowed to `Data Foundation only`; current live write scope is `H:\Malf-Pas-data\market_base_day.duckdb / market_base_week.duckdb / market_base_month.duckdb` only.
...
Roadmap 2 is now active-after-card-022 on the `data-foundation` live route.
